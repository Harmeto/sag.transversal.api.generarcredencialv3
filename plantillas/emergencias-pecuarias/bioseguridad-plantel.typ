// Plantilla "Bioseguridad Plantel" (Pauta Bioseguridad en planteles de aves,
// F-VYC-VIS-PP-004) migrada desde v2 (emergencias-pecuarias/bioseguridad-plantel.html
// + JavaScript) a Typst. Es la misma estructura que bioseguridad-traspatio.typ; sólo
// cambian el título, el código del formulario y las filas de la tabla de datos.
//
// Recibe en `sys.inputs.datos` el mismo `DatosCredencial` que arma el BFF de
// Emergencias Pecuarias (BioseguridadMapper.toDatosCredencialPlantel):
//   nombreCompleto, nombreEstablecimiento, rup, responsableBioseguridad,
//   tipoInfraestructura, cantidadAves, numeroPabellones, rubro,
...
// El resto de claves del mapper (folio, nombreFirmante, porcentajeCumplimiento,
// s1_obs..., tipoProduccion, nombreCalle, nombreRegion...) no se imprimen, igual que en v2.
//
...
#let encabezado = context table(
  ...
  pad(y: 5pt, {
    ...
    block(inset: (x: 9pt), text(size: 7pt)[Pauta Bioseguridad en planteles de aves])
  }),
  // código / versión / vigencia / página
  pad(x: 8pt, y: 5pt, {
    set text(size: 7pt)
    set par(leading: 4.5pt)
    [*Código:* F-VYC-VIS-PP-004 \
     *Versión:* 02 \
...
// ---------- Datos del plantel + QR (.layout-row) ----------
...
#grid(
  ...
  table(
    ...
    ..fila-dato("RUP", val("rup")),
    ..fila-dato("Titular", val("nombreCompleto")),
    ..fila-dato("Nombre del establecimiento", val("nombreEstablecimiento")),
    ..fila-dato("Responsable bioseguridad del plantel", val("responsableBioseguridad")),
    ..fila-dato("Tipo infraestructura", val("tipoInfraestructura")),
    ..fila-dato("Cantidad de aves", val("cantidadAves")),
    ..fila-dato("N° de pabellones según tipo de infraestructura", val("numeroPabellones")),
    ..fila-dato("Rubro", val("rubro")),
  ),
